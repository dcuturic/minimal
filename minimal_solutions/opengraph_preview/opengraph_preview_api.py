from flask import Blueprint, request
from app.responses import success_response, error_response, ErrorCodes
from minimal_solutions.opengraph_preview.opengraph_preview_validation import validate_request
import html

api_bp = Blueprint('opengraph_preview_api', __name__)

@api_bp.route('/api/minimal-solutions/opengraph_preview', methods=['POST'])
def generate():
    data = request.get_json(silent=True)
    if data is None:
        return error_response(
            code=ErrorCodes.BAD_REQUEST,
            message="Erwartet wurde ein JSON-Objekt/Dictionary."
        )

    is_valid, errors = validate_request(data)
    if not is_valid:
        return error_response(
            code=ErrorCodes.VALIDATION_ERROR,
            message="Validierungsfehler",
            details=errors
        )

    title = data.get('title', '').strip() if data.get('title') else ''
    description = data.get('description', '').strip() if data.get('description') else ''
    image_url = data.get('image_url', '').strip() if data.get('image_url') else ''
    domain = data.get('domain', '').strip() if data.get('domain') else ''

    tags = []
    
    tags.append('<meta property="og:type" content="website">')
    
    if domain:
        url = domain if domain.startswith('http') else f"https://{domain}"
        tags.append(f'<meta property="og:url" content="{html.escape(url)}">')
    
    if title:
        tags.append(f'<meta property="og:title" content="{html.escape(title)}">')
        
    if description:
        tags.append(f'<meta property="og:description" content="{html.escape(description)}">')
        
    if image_url:
        tags.append(f'<meta property="og:image" content="{html.escape(image_url)}">')

    html_output = "\n".join(tags)

    return success_response(data={
        "html": html_output,
        "preview_data": {
            "title": title,
            "description": description,
            "image_url": image_url,
            "domain": domain
        }
    })
