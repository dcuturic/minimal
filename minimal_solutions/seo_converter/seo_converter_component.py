...
def render_seo_converter_component():
    return Div(
        # Hero Section
        Div(
            H2("SEO Converter", cls="text-3xl font-bold text-white mb-2 tracking-tight"),
            P("SEO Converter konvertiert SEO-Daten als kleine UI-Demo und stellt dieselbe Funktion als API-Schnittstelle bereit.", cls="text-gray-400 text-sm"),
            cls="mb-8"
        ),
        ...
        Div(
            ...
            Div(
                Form(
                    # Topic
                    Div(
                        Label("Eingabedaten (Topic / JSON / Text)", cls="block text-xs font-medium text-gray-400 mb-1 uppercase tracking-wider"),
                        Textarea(
                            name="topic", 
                            placeholder="Gib hier die zu konvertierenden Daten ein...", 
                    ...
                    # Target
                    Div(
                        Label("Zielformat (Target)", cls="block text-xs font-medium text-gray-400 mb-1 uppercase tracking-wider"),
                        Input(
                            type="text",
                            name="target",
                            placeholder="z.B. CSV, JSON, XML",
                    ...
                    Div(
                        ...
                        Input(
                            type="text",
                            name="options",
                            placeholder="z.B. pretty_print, include_headers (kommagetrennt)",
                            cls="w-full bg-[#0a0a0f] border border-gray-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner"
                        ),
                        cls="mb-8"
                    ),
            ...
            Div(
                ...
                Div(
                    ...
                    P("Konvertiere Daten...", cls="text-gray-400 text-sm font-medium tracking-wide animate-pulse"),
                ...
                Div(
                    ...
                    Div(
                        ...
                        P("die konvertierten SEO-Daten zu sehen.", cls="text-gray-500 text-sm"),
        ...
        Div(
            ...
            P("Du kannst den SEO Converter auch per API ansprechen. Hier ist ein Beispiel für Request und Response:", cls="text-gray-400 mb-4"),
            Div(
                Div(
    ...
    "result": "<title>My Title</title>"
        ...
        Script(r"""
            document.addEventListener('DOMContentLoaded', function() {
                const form = document.getElementById('seo-converter-form');
                ...
                if(form) {
                    form.addEventListener('submit', async function(e) {
                        ...
                        try {
                            const response = await fetch('/api/minimal-solutions/seo_converter', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json'
                                },
                            ...
                            let html = `
                                <div class="bg-[#1a1a24] p-4 rounded-xl border border-gray-800 h-full flex flex-col relative group flex-1">
                                    <h4 class="text-indigo-400 font-medium mb-3 text-sm uppercase tracking-wider">Konvertiertes Ergebnis</h4>
                            ...
                            formattedResult.querySelector('.download-btn').addEventListener('click', function() {
                                ...
                                if (typeof window.triggerDownload === 'function') {
                                    window.triggerDownload(resultText, ext, 'seo_converter_data', this);
                                } else {
                                    ...
                                    a.href = url;
                                    a.download = `seo_converter_data.${ext}`;
